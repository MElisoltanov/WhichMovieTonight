...
For this portfolio project, we chose to build **WhichMovieTonight**, a web application that helps users quickly find a movie without wasting time browsing across multiple streaming platforms. 

The goal is simple: gather everything in one place-movie info, user reviews, and streaming availability-to make the movie-picking process easier and faster.
...
In **stage 2** you will find a detailed timeline of our project:
Highlighting each phase, its key milestones, and expected deliverables to ensure a clear roadmap from the current stage through to project closure.
